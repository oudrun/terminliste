...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'addTrial') {
        ...
        $stmt->execute([
            trim($_POST['trial_number'] ?? ''),
            trim($_POST['title'] ?? ''),
            trim($_POST['organizer'] ?? ''),
            trim($_POST['location'] ?? ''),
            trim($_POST['trial_place'] ?? ''),
            $_POST['date_start'] ?? '',
            $_POST['date_end'] ?? '',
            ...
            trim($_POST['responsible_club'] ?? ''),
        ]);
        flash('Prøven ble lagt til.');
        redirect('admin.php');
    }
    ...
    if ($action === 'addClass') {
        ...
        flash('Klassen ble lagt til.');
        redirect('admin.php');
    }

    if ($action === 'updateStatus') {
        $stmt = $pdo->prepare('UPDATE registrations SET status = ? WHERE id = ?');
        $stmt->execute([$_POST['status'] ?? 'pending', (int) ($_POST['registration_id'] ?? 0)]);
        flash('Påmeldingsstatus ble oppdatert.');
        redirect('admin.php');
    }

    if ($action === 'deleteTrial') {
        $stmt = $pdo->prepare('DELETE FROM dog_trials WHERE id = ?');
        $stmt->execute([(int) ($_POST['trial_id'] ?? 0)]);
        flash('Prøven ble slettet.', 'success');
        redirect('admin.php');
    }

    if ($action === 'deleteClass') {
        $stmt = $pdo->prepare('DELETE FROM trial_classes WHERE id = ?');
...
<header>
    <div class="container">
        <h1>Administrasjon av terminliste</h1>
        <p>Legg inn prøver, klasser og følg opp påmeldinger.</p>
        <nav>
